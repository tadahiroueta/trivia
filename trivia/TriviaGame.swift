...
class TriviaGame {
    static let database = getQuestionDB()
    static let categoryNames: [String] = Array(database.keys).sorted()
    static var game: TriviaGame?

    let categoryName: String, quantity: Int
    var trivias: [Trivia] = [], latestTrivia: Trivia?, points: Int = 0
    
    init(categoryName: String, quantity: Int) {
        self.categoryName = categoryName
        self.quantity = quantity
        
        var category = TriviaGame.database[categoryName]!
        category.shuffle()
        
        for _ in 0..<quantity { trivias.append(category.removeFirst()) }
        TriviaGame.game = self
    }
    
    func getTrivia() -> Trivia {
        latestTrivia = trivias.removeFirst()
        return latestTrivia!
    }
    
    func getCheck(answer: String) -> (Bool, String) {
        let correct = latestTrivia!.correct
        guard answer == correct else { return (false, correct) }
        points += 1
        return (true, correct)
    }
    
    func didGameEnd() -> (Int, Int)? {
        guard trivias.isEmpty else { return nil }
